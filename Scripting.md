...
| listen      | text     | -       | -        | -           | Check if sound is perceived at the location of 'who' character. Triggers nested actions if any. |
| lookFor     | text     | -       | -        | -           | Look for some characters around another, with a desired radius. The first discovered one become the new context for nested action elements (except if you place a 'changeContext' attribute at false, which will make you keep the current context). Field 'info' precise a PersoInfo enum, and 'negative' could be used to detect the opposite: character away from another. 'radius' allow to specify a number of tile around to look for. 'sight' at 'true' means that found character should be looking us. |
| loop        | text     | -       | -        | -           | Simple loop, with optional exit condition, indicated by 'when' attribute. |
