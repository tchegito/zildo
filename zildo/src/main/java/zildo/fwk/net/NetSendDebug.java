...
public class NetSendDebug extends NetSend {
	...
	public NetSendDebug(InetAddress p_address, int p_port) {
		...
		filename = p_address == null ? "localhost" : p_address.getHostName()
				+ "_";
	...
	private void logPacket(boolean p_in, Packet p_packet,
			TransferObject p_target) {
		String targetStr = p_target == null ? "broadcast" : p_target.address
				.toString();
		String sens = p_in ? "<<" : ">>";
		...
		logBuffer.put(" " + df.format(new Date()) + " " + p_packet.getType()
				+ complement + " " + sens + " " + targetStr + " size=" + size
				+ "\n");
	}
