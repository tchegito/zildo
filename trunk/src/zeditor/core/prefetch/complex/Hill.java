...
public class Hill extends DelegateDraw {
	...
	// Collines
	final int[][] tab_raise=
	            {{0,       1+anglea,1+angle0,1+angleb,0       },
		...
		for (int i=0;i<4;i++) {
			for (int j=0;j<4;j++) {
	            ...
	            if (tab_raise[i][j]!=0 && etage<=(tab_raise[i][j] & 3)) {
	            	// On ne remplit cette case que si c'est pour rehausser le terrain}
		            ...
		            if ((value==0 && etage!=3) ||     // rien avant?}
		            	...
		            	// Pour ça, on repère une case adjacente de même hauteur}
		            	int cur_angle=-2;
		            	while (true) {
			                cur_angle=cur_angle+2;
			                value=mapetage[y+movy[cur_angle]][x+movx[cur_angle]];
			                cur_etage=value & 3;
			            	if (cur_etage==etage || cur_angle==6) {
			            		break;
			            	}
		            	}
		            	if (cur_etage!=etage) { // ProblŠme, ‡a ne doit pas arriver !}
		            		mapetage[y][x]+=angle_to_compare;
		            	} else {
		            		// On a trouv‚ cette case de mˆme hauteur, on v‚rifie que les
			                ...
			                if (cur_angle % 2 == value) {
			                	...
			                	if (angle>angle7 && (mapetage[y][x] & 3)==2) {
				                   ...
				                   if      (angle==angle8)  angle=angle3;
				                   else if (angle==angle9)  angle=angle5;
				                   else if (angle==anglea)  angle=angle7;
				                   else if (angle==angleb)  angle=angle1;
			                	}
			                	mapetage[y][x]+=angle;
			                }
		            	}
			...
			for (int j=0;j<dx-1;j++) {
				if (mapetage[i][j] != 0) {
					...
					if ((temp & 128) != 0) {
						...
						int temp2=0;	//Le bas des collines prend un motif de plus que les autres côtés
						...
						temp2=0;	// Un motif de plus sur le côté haut des collines
						if (temp == 0) {
							temp2=1;
