...
public class ZildoCanvas extends AWTOpenGLCanvas {
	...
	/**
	 * Action launched by right-clicking on the map. Depends on the kind of
	 * selection.<br/>
	...
	public void clearWithBrush(Point p) {
		...
		case TILES:
			...
			int nTile = EngineZildo.getMapManagement().getCurrentMap().getAtmosphere().getEmptyTile();
		...
		case CHAININGPOINT:
			...
			if (ch != null) {
				EngineZildo.getMapManagement().getCurrentMap().removeChainingPoint(ch);
		...
		case PERSOS:
			...
			for (SpriteEntity entity : entities) {
				...
				if (sel.getKind() == SelectionKind.PERSOS) {
					...
					// Remove dialogs too
					EngineZildo.getMapManagement().getCurrentMap().getMapDialog().removePersoDialog(perso.getName());
		...
		if (!errors.isEmpty()) {
			for (ErrorDescription error : errors) {
				if (error.autofix) {
					...
					if (0 == JOptionPane.showConfirmDialog(this, error.message + "\n\nDo you want ZEditor to fix this ?", "ZEditor", JOptionPane.YES_NO_OPTION)) {
						error.fixAction.run();
					}
				}
			}
	...
	/**
	 * Ask the server side to load the given map. If we come from a chaining
	 * point, then we return the target point.
	 * 
	 * @param p_mapName
	 * @param p_fromChangingPoint
	 *            (optional)
	...
	public ChainingPoint loadMap(String p_mapName, ChainingPoint p_fromChangingPoint) {
		...
		if (p_fromChangingPoint != null) {
			...
			if (ch != null) {
				...
				p.x = z.getX1() - ZildoScrollablePanel.viewSizeX / 2;
				p.y = z.getY1() - ZildoScrollablePanel.viewSizeY / 2;
			}
		}
		panel.setPosition(p);
	...
	public void clearMap() {
		EngineZildo.getMapManagement().clearMap();
		changeMap = true;
	}
	...
	/**
	 * Detect which object of current kind (sprite, chaining point ...) is under
	 * the mouse
	 * 
	 * @param p
	 */
	public void setObjectOnCursor(Point p) {
		...
		case CHAININGPOINT:
			Area map = EngineZildo.getMapManagement().getCurrentMap();
			List<ChainingPoint> points = EngineZildo.getMapManagement().getCurrentMap().getChainingPoints();
	...
	/**
	 * Returns a set of entities according to a point location, or inside a
	 * zone.<br/>
	...
	private List<SpriteEntity> findEntity(SelectionKind p_kind, Point p_point, Zone p_zone) {
		...
		for (SpriteEntity entity : sprites) {
			...
			if ((typ == EntityType.PERSO && p_kind == SelectionKind.PERSOS) || (typ != EntityType.PERSO && p_kind == SelectionKind.SPRITES)) {
