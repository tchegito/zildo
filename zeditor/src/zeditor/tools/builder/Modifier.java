 ...
 * 
 * We can find here a bunch of methods dealing some connex issues like :
 * <ul>
...
public class Modifier {
	...
	public void saveElements2() {
		...
		for (Zone z : elements) {
			bankElem.addSprFromImage(nSpr, z.getX1(), z.getY1(), z.getX2(), z.getY2());
			nSpr++;
		}
	...
	public void saveElements3() {
		...
		// bankElem.removeSpr(ElementDescription.WINDOW_WOOD.getNSpr());
		// SPADE : 0,177,11,19
		// bankElem.addSprFromImage(ElementDescription.BOOK_SIGN.getNSpr(), 11,
		// 177, 32, 20);
		// bankElem.addSprFromImage(ElementDescription.LEAF.getNSpr(), 16, 169,
		// 8, 7);
		// bankElem.addSprFromImage(ElementDescription.MILK.getNSpr(), 43, 181,
		// 11, 16);
	...
	public void saveZildo() {
		...
		/*
		 * bankZildo.loadImage("link3b", COLOR_BLUE); Zone[] zones = new
		 * PjZildo().getZones(); int nSpr=bankZildo.getNSprite(); for (Zone z :
		 * zones) { if (bankZildo.getNSprite() <= nSpr) {
		 * bankZildo.addSprFromImage(nSpr, z.x1, z.y1, z.x2, z.y2); } nSpr++; }
		 */
		bankZildo.saveBank();
	}
	...
	public void saveFontes2() {
		...
		String fontOrder = "KRWXMLNQFDAUTYHBVG." + "EOPwCZJSmIfnd7?xüuvûyù40pq-k%" + "9a6z£àâ5oceghj~$23çèéêë8r§#bsi!>ïtl1)</:" + "',(äîôö";
		...
		for (Zone z : elements) {
			bankFont.addSprFromImage(nSpr, z.getX1(), z.getY1(), z.getX2(), z.getY2());
			nSpr++;
		}
		bankFont.saveBank();
	}
		...
		new AllMapProcessor() {
			...
			public boolean run() {
				...
				for (SpriteEntity entity : entities) {
					if (entity.getEntityType() == EntityType.ELEMENT || entity.getEntityType() == EntityType.ENTITY) {
